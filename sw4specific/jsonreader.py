...
for line in d['movie_script']:
	scr_text = line['text']
	scr_text = scr_text.replace('artoo', 'r2')
	scr_text = scr_text.replace('detoo', 'd2')
	scr_text = scr_text.replace('Artoo', 'r2')
	scr_text = scr_text.replace('Detoo', 'd2')

	scr_text_arr = list(scr_text)
	st_idx = len(scr_text_arr)-1
	while st_idx >= 0:
		if not(scr_text_arr[st_idx].isalnum() or scr_text_arr[st_idx] == ' '):
			scr_text_arr[st_idx] = ''
		st_idx -= 1

	scr_text = ''.join(scr_text_arr)
	scr_text = scr_text.lower()
	line['text'] = scr_text
